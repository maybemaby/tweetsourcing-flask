...
def categorize_news(results_object: dict, tweet_kwords: list, *args):
    ...
    if len(args) > 0:
        ...
        article_kword_gen = extract_articles(
            [article.link for article in article_results]
        )
...
def extract_articles(url_list):
    ...
    """
    articles = [
        Article(url, memoize_articles=True, fetch_images=False) for url in url_list
    ]
...
def keyword_compare(kwords1: list, kwords2: list):
    ...
    if type(kwords2) != list or len(kwords2) == 0:
        raise TypeError(
            f"keyword_compare only takes an iterable list of words, {repr(kwords2)} is not compatible"
        )
    matches = 0
